...
The {\tt doRedis} package allows for dynamic pools of back-end workers. New
...
\noindent
Thus, this parallel loop consists of 10 iterations (tasks) using the 
...
to place each task in the work queue.  Finally, each iteration runs the Monte
Carlo estimation for 1,000,000 points.
...
directly in the {\tt foreach} function.
...
We ran the following R code on the ``master'' workstation after starting
...
The multiple is chosen to separate seed initialization values as widely as
possible. This simple scheme is sufficient for many problems, and comparable to
...
workers explicitly in the {\tt foreach} loop. The following example
illustrates its use.
