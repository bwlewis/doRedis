...
GNU/Linux systems, it works equally well on Mac OS X and other Unix systems and
we even have available installers that configure Redis and {\tt doRedis} as
Microsoft Windows services. Back end parallel R worker processes are
effectively anonymous--they will run as long as all the R package dependencies
