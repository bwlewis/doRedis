...
across different operating systems. Although {\tt doRedis} was developed on
GNU/Linux systems, it works equally well on other Unix systems like Mac OS X
and even on Windows systems.  Back end parallel R worker processes are
effectively anonymous--they may run anywhere as long as all the R package
dependencies required by the task at hand are available.
...
\htmladdnormallink{http://github.com/antirez/redis/tarball/1.3.6}{http://github.com/antirez/redis/tarball/1.3.6}, with source code available from Github at
\htmladdnormallink{http://github.com/antirez/redis}{http://github.com/antirez/redis}. A Windows version of Redis is available from:
\htmladdnormallink{https://github.com/dmajkic/redis}{https://github.com/dmajkic/redis}.
...
The doRedis package for R is available for all supported R platforms.
...
The example bootstrapping code is shown in in the listing below.
...
Listing 5 presents a parallel capable variation of the {\tt boot} function in
...
Listing 6 illustrates a simple user-defined seed function.
