...
avoiding loading it all in memory and overlapping I/O and computation.
...
specified work queue. doRedis workers listenening on that queue will then
...
All options can be set using the standard {\tt foreach} interface or by using
the special \verb+setProgress()+, \verb+setChunkSize()+, \verb+setExport()+,
\verb+setPackages()+, and \verb+setReduce()+ functions. Thus,
...
The package provides these duplicate functions to facilitate working with other
R packages that use \verb+foreach+ internally. In such cases it is not
practical to directly supply arguments to the \verb+foreach+ invocation within
the package. The \verb+set*+ functions provide a way to register an external
\verb+foreach+ back end and control its behavior.

Here is a list of all the options.

\begin{itemize}
\item {\tt chunksize} break the loop iterations up into chunks of at most the specified size, equivalent to {\tt chunkSize}
\item {\tt progress} display an R progress bar
\item {\tt packages} character vector of required R packages
\item {\tt export} character vector of variable names to explicitly export
\item {\tt reduce} optional per-task reduction function
\item {\tt stream} optionally stream work as its read
\end{itemize}

The last two are new and not as well tested/used/known as older parts of the
package, use with care. In particular, as of this writing, the {\tt stream}
option does not support fault tolerance.
