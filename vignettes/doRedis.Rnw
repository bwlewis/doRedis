...
{\tt foreach} back ends support this but {\tt doRedis} does).
...
The following code listing presents a contrived but entirely self-contained
...
\verb+replicate+.  Each iteration is
