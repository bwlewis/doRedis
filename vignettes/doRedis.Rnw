...
parameter, if it exists, is the loop variable value. (It is possible to use
...
a special Redis key called a ``list'' that supports blocking reads. Users
...
generator prior to the \verb+foreach+ loop.
