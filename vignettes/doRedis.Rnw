...
keys used by a master and worker R processes for a work queue named ``jobs'',
...
\end{center}
\caption{Example {\tt doRedis} keys for an example work queue named ``jobs''}
\label{keys}
\end{figure}
...
Along with the work queue, a counter key named ``jobs.count'' is set up to
